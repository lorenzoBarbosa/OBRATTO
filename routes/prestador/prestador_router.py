...
from config import templates  # <-- CORREÇÃO: Importa a instância única de templates

# --- Bloco de Simulação (substitua pela sua lógica de banco de dados) ---
class Servico:
...
class ServicoRepo:
    def __init__(self):
        self._servicos: List[Servico] = [
            Servico(1, 1, "Construção de Parede", "Levantamento de paredes com tijolos.", "Construção", 1500.00, True),
            Servico(2, 1, "Instalação Elétrica", "Passagem de fiação e instalação de tomadas.", "Elétrica", 3200.50, False),
            Servico(3, 1, "Pintura Interna", "Pintura de paredes e tetos.", "Pintura", 850.00, True),
        ]
        self._next_id = 4
...
# A linha 'templates = Jinja2Templates(directory="templates")' foi REMOVIDA daqui.

def obter_id_prestador_logado() -> int:
    return 1

# --- Rotas do Prestador ---
@router.get("/", response_class=HTMLResponse, name="painel_prestador")
async def painel_prestador(request: Request, id_prestador: int = Depends(obter_id_prestador_logado)):
    return templates.TemplateResponse("prestador/painel.html", {"request": request, "id_prestador": id_prestador, "pagina_ativa": "painel"})

@router.get("/servicos", response_class=HTMLResponse, name="listar_servicos")
async def listar_servicos(request: Request, id_prestador: int = Depends(obter_id_prestador_logado), q: Optional[str] = None):
    servicos = servico_repo.obter_servicos_por_prestador(id_prestador)
    if q: servicos = [s for s in servicos if q.lower() in s.titulo.lower()]
    return templates.TemplateResponse("prestador/servicos_listar.html", {"request": request, "servicos": servicos, "q": q, "id_prestador": id_prestador, "pagina_ativa": "servicos"})

@router.get("/servicos/novo", response_class=HTMLResponse, name="form_novo_servico")
async def form_novo_servico(request: Request, id_prestador: int = Depends(obter_id_prestador_logado)):
    return templates.TemplateResponse("prestador/servico_form.html", {"request": request, "acao": "novo", "id_prestador": id_prestador, "pagina_ativa": "servicos"})
...
# ... (Restante das rotas GET e POST para editar, remover, agenda, etc. continuam aqui, sem alterações na lógica) ...
@router.get("/servicos/{id_servico}/editar", response_class=HTMLResponse, name="form_editar_servico")
async def form_editar_servico(request: Request, id_servico: int, id_prestador: int = Depends(obter_id_prestador_logado)):
    servico = servico_repo.obter_servico_por_id(id_servico)
    if not servico or servico.id_prestador != id_prestador: raise HTTPException(status_code=404)
    return templates.TemplateResponse("prestador/servico_form.html", {"request": request, "servico": servico, "acao": "editar", "id_prestador": id_prestador, "pagina_ativa": "servicos"})
...
async def remover_servico(request: Request, id_servico: int, id_prestador: int = Depends(obter_id_prestador_logado)):
    servico = servico_repo.obter_servico_por_id(id_servico)
    if not servico or servico.id_prestador != id_prestador: raise HTTPException(status_code=404)
...
async def agenda_prestador(request: Request, id_prestador: int = Depends(obter_id_prestador_logado)):
    return templates.TemplateResponse("prestador/agenda.html", {"request": request, "id_prestador": id_prestador, "pagina_ativa": "agenda"})

@router.get("/assinatura", response_class=HTMLResponse, name="assinatura_prestador")
async def assinatura_prestador(request: Request, id_prestador: int = Depends(obter_id_prestador_logado)):
    return templates.TemplateResponse("prestador/assinatura.html", {"request": request, "id_prestador": id_prestador, "pagina_ativa": "assinatura"})

@router.get("/solicitacoes", response_class=HTMLResponse, name="solicitacoes_prestador")
async def solicitacoes_prestador(request: Request, id_prestador: int = Depends(obter_id_prestador_logado)):
    return templates.TemplateResponse("prestador/solicitacoes.html", {"request": request, "id_prestador": id_prestador, "pagina_ativa": "solicitacoes"})
