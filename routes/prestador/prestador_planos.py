...
# Rota para a página principal de planos
...
async def exibir_formulario_edicao(request: Request):
...
async def processar_edicao_assinatura(request: Request, id_fornecedor: int = Form(...), id_plano: int = Form(...)):
    try:
        assinatura_ativa = inscricao_plano_repo.obter_inscricao_por_prestador(id_prestador=id_fornecedor)
...
# Rota para EXIBIR a página de renovação
...
# Rota para PROCESSAR a renovação do plano
@router.post("/planos/renovar", name="processar_renovacao_plano")
async def processar_renovacao_plano(request: Request, id_prestador: int = Form(...)):
    ...
    try:
        ...
        print(f"Erro inesperado ao renovar plano: {e}") 
...
# Rota para EXIBIR a página de cancelamento
@router.get("/planos/cancelar", name="exibir_pagina_cancelamento")
async def exibir_pagina_cancelamento(request: Request):
    return templates.TemplateResponse("prestador/planos_cancelar.html", {"request": request})


# Rota para PROCESSAR o pedido de cancelamento 
@router.post("/planos/cancelar", name="processar_cancelamento_plano")
async def processar_cancelamento_plano(request: Request, id_prestador: int = Form(...), confirmacao: str = Form(...)):
    try:
        ...
        if confirmacao.lower() == "confirmar":
            sucesso = inscricao_plano_repo.deletar_inscricao_por_prestador(id_prestador=id_prestador)
